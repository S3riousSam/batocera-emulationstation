...
void GuiMenu::createInputTextRow(GuiSettings* gui, std::string title, const char* settingsID, bool password)
{
	...
	ed = std::make_shared<TextComponent>(window,
		((password && RecalboxConf::getInstance()->get(settingsID) != "") ? "*********" : RecalboxConf::getInstance()->get(settingsID)),
	...
	row.makeAcceptInputHandler([this, title, updateVal, settingsID] {
		...
		{
			mWindow->pushGui(new GuiTextEditPopupKeyboard(mWindow, title, RecalboxConf::getInstance()->get(settingsID), updateVal, false));
		}
		else
		{
			mWindow->pushGui(new GuiTextEditPopup(mWindow, title, RecalboxConf::getInstance()->get(settingsID), updateVal, false));
		}
	});
	gui->addRow(row);
}
	...
	, mVersion(window)
{
		...
		addEntry(_("SYSTEM SETTINGS").c_str(), 0x777777FF, true, [this] {
			...
			// system informations
			{
				...
				std::function<void()> openGui = [this] {
					...
					auto version = std::make_shared<TextComponent>(
						mWindow, RecalboxSystem::getInstance()->getVersion(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
					...
					auto space = std::make_shared<TextComponent>(
						mWindow, RecalboxSystem::getInstance()->getFreeSpaceInfo(), Font::get(FONT_SIZE_MEDIUM), warning ? 0xFF0000FF : 0x777777FF);
					...
					// support
					{
						...
						auto supportFile =
							std::make_shared<TextComponent>(mWindow, _("CREATE A SUPPORT FILE"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
			...
			// Updates
			{
				...
				std::function<void()> openGuiD = [this] {
					...
					// Start update
					{
						...
						auto update = std::make_shared<TextComponent>(mWindow, _("START UPDATE"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
						auto bracket = makeArrow(mWindow);
				...
				auto update = std::make_shared<TextComponent>(mWindow, _("UPDATES"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
			...
			// backup
			{
				...
				auto backupSettings = std::make_shared<TextComponent>(mWindow, _("BACKUP USER DATA"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
			...
			// Kodi
			{
				...
				std::function<void()> openGui = [this] {
					...
					kodiAtStart->setState(RecalboxConf::getInstance()->get("kodi.atstartup") == "1");
				...
				row.makeAcceptInputHandler(openGui);
				auto kodiSettings = std::make_shared<TextComponent>(mWindow, _("KODI SETTINGS"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
			...
			// install
			{
				...
				auto installSettings =
					std::make_shared<TextComponent>(mWindow, _("INSTALL BATOCERA ON A NEW DISK"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
			...
			// Security
			{
				...
				std::function<void()> openGui = [this] {
					...
					auto rootpassword = std::make_shared<TextComponent>(
						mWindow, RecalboxSystem::getInstance()->getRootPassword(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				...
				row.makeAcceptInputHandler(openGui);
				auto securitySettings = std::make_shared<TextComponent>(mWindow, _("SECURITY"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
	...
	addEntry(_("GAMES SETTINGS").c_str(), 0x777777FF, true,
		[this] {
			...
			shaders_choices->setSelectedChangedCallback(
				[integerscale_enabled](std::string selectedShader) { integerscale_enabled->setState(selectedShader != "none"); });
			...
			if (RecalboxConf::getInstance()->get("system.es.menu") != "bartop")
			{
				// Retroachievements
				{
					...
					std::function<void()> openGui = [this] {
						...
						retroachievements_hardcore_enabled->setState(RecalboxConf::getInstance()->get("global.retroachievements.hardcore") == "1");
						...
						retroachievements->addSaveFunc([retroachievements_enabled, retroachievements_hardcore_enabled] {
							...
							RecalboxConf::getInstance()->set(
								"global.retroachievements.hardcore", retroachievements_hardcore_enabled->getState() ? "1" : "0");
							RecalboxConf::getInstance()->saveRecalboxConf();
						});
					...
					auto retroachievementsSettings =
						std::make_shared<TextComponent>(mWindow, _("RETROACHIEVEMENTS SETTINGS"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
					auto bracket = makeArrow(mWindow);
				...
				// Bios
				{
					std::function<void()> openGuiD = [this, s] {
						...
						if (biosInformations.size() == 0)
						{
							...
							auto biosText =
								std::make_shared<TextComponent>(mWindow, _("NO MISSING BIOS").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
						...
						else
						{
							for (auto systemBios = biosInformations.begin(); systemBios != biosInformations.end(); systemBios++)
							{
								...
								auto biosText =
									std::make_shared<TextComponent>(mWindow, (*systemBios).name.c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
								...
								std::function<void()> openGuiDBios = [this, systemBiosData] {
									...
									{
										auto biosPath =
											std::make_shared<TextComponent>(mWindow, biosFile->path.c_str(), Font::get(FONT_SIZE_MEDIUM), 0x000000FF);
										auto biosMd5 =
											std::make_shared<TextComponent>(mWindow, biosFile->md5.c_str(), Font::get(FONT_SIZE_SMALL), 0x777777FF);
										auto biosStatus = std::make_shared<TextComponent>(
											mWindow, biosFile->status.c_str(), Font::get(FONT_SIZE_SMALL), 0x777777FF);
					...
					ComponentListRow row;
					row.makeAcceptInputHandler(openGuiD);
					auto bios = std::make_shared<TextComponent>(mWindow, _("MISSING BIOS"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
					auto bracket = makeArrow(mWindow);
				...
				// Custom config for systems
				{
					...
					std::function<void()> openGuiD = [this, s] {
						...
						for (auto system = systems.begin(); system != systems.end(); system++)
						{
							if ((*system) != SystemData::getFavoriteSystem())
							{
								...
								auto systemText =
									std::make_shared<TextComponent>(mWindow, (*system)->getFullName(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
								auto bracket = makeArrow(mWindow);
								systemRow.addElement(systemText, true);
								systemRow.addElement(bracket, false);
								SystemData* systemData = (*system);
								systemRow.makeAcceptInputHandler([this, systemData] { popSystemConfigurationGui(systemData, ""); });
					...
					// Advanced button
					row.makeAcceptInputHandler(openGuiD);
					auto advanced = std::make_shared<TextComponent>(mWindow, _("ADVANCED"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
					auto bracket = makeArrow(mWindow);
	...
	if (RecalboxConf::getInstance()->get("system.es.menu") != "bartop")
	{
		addEntry(_("UI SETTINGS").c_str(), 0x777777FF, true, [this] {
			...
			s->addSaveFunc([this, optionsVideo, currentDevice] {
				if (currentDevice != optionsVideo->getSelected())
				{
					...
					RecalboxConf::getInstance()->saveRecalboxConf();
					this->mWindow->pushGui(new GuiMsgBox(this->mWindow, _("THE SYSTEM WILL NOW REBOOT"), _("OK"), [] {
						if (runRestartCommand() != 0)
						{
							LOG(LogWarning) << "Reboot terminated with non-zero result!";
						}
	...
	addEntry(_("SOUND SETTINGS").c_str(), 0x777777FF, true, [this] {
	...
	if (RecalboxConf::getInstance()->get("system.es.menu") != "bartop")
	{
		addEntry(_("NETWORK SETTINGS").c_str(), 0x777777FF, true, [this] {
			Window* window = mWindow;

			auto s = new GuiSettings(mWindow, _("NETWORK SETTINGS").c_str());
			auto status = std::make_shared<TextComponent>(
				mWindow, RecalboxSystem::getInstance()->ping() ? _("CONNECTED") : _("NOT CONNECTED"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
			s->addWithLabel(_("STATUS"), status);
			auto ip = std::make_shared<TextComponent>(mWindow, RecalboxSystem::getInstance()->getIpAdress(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
	...
	{
		// manual or automatic ?
		addEntry(_("SCRAPE").c_str(), 0x777777FF, true, [this] {
			...
			// automatic
			{
				std::function<void()> autoAct = [this] {
					...
					window->pushGui(new GuiMsgBox(
						window, _("REALLY SCRAPE?"), _("YES"), [window] { window->pushGui(new GuiAutoScrape(window)); }, _("NO"), nullptr));
				};

				ComponentListRow row;
				row.makeAcceptInputHandler(autoAct);
				auto sc_auto = std::make_shared<TextComponent>(mWindow, _("AUTOMATIC SCRAPER"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
			...
			// manual
			{
				...
				auto sc_manual = std::make_shared<TextComponent>(mWindow, _("MANUAL SCRAPER"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
				auto bracket = makeArrow(mWindow);
...
void GuiMenu::popSystemConfigurationGui(SystemData* systemData, std::string previouslySelectedEmulator) const
{
	...
	selected = false;
	for (auto emulator = systemData->getEmulators()->begin(); emulator != systemData->getEmulators()->end(); emulator++)
	{
	...
	smoothing_enabled->setState(
		RecalboxConf::getInstance()->get(systemData->getName() + ".smooth", RecalboxConf::getInstance()->get("global.smooth")) == "1");
...
{
	for (size_t i = 0; i < mLoadedInput.size(); i++)
	{
