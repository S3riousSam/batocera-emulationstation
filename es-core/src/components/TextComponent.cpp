...
TextComponent::TextComponent(Window* window, const std::string& text, const std::shared_ptr<Font>& font, unsigned int color, Alignment align,
	Eigen::Vector3f pos, Eigen::Vector2f size)
	: GuiComponent(window)
	, mFont(NULL)
	, mUppercase(false)
	, mColor(0x000000FF)
...
void TextComponent::render(const Eigen::Affine3f& parentTrans)
{
	...
	if (mTextCache)
	{
		...
		if (Settings::getInstance()->getBool("DebugText"))
		{
			...
			case ALIGN_CENTER:
				Renderer::drawRect(
					(mSize.x() - mTextCache->metrics.size.x()) / 2.0f, 0.0f, mTextCache->metrics.size.x(), mTextCache->metrics.size.y(), 0x00000033);
				break;
			case ALIGN_RIGHT:
				Renderer::drawRect(
					mSize.x() - mTextCache->metrics.size.x(), 0.0f, mTextCache->metrics.size.x(), mTextCache->metrics.size.y(), 0x00000033);
				break;
			}
		}
...
void TextComponent::onTextChanged()
{
	...
	if (!isMultiline && mSize.x() && text.size() && (size.x() > mSize.x() || addAbbrev))
	{
		...
		mTextCache = std::shared_ptr<TextCache>(
			f->buildTextCache(text, Eigen::Vector2f(0, 0), (mColor >> 8 << 8) | mOpacity, mSize.x(), mAlignment, mLineSpacing));
	}
	else
	{
		mTextCache = std::shared_ptr<TextCache>(f->buildTextCache(
			f->wrapText(text, mSize.x()), Eigen::Vector2f(0, 0), (mColor >> 8 << 8) | mOpacity, mSize.x(), mAlignment, mLineSpacing));
	}
}
