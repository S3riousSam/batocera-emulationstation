...
GuiMsgBox::GuiMsgBox(Window* window, const std::string& text, const std::string& name1, const std::function<void()>& func1, const std::string& name2,
	const std::function<void()>& func2, const std::string& name3, const std::function<void()>& func3, Alignment align)
	: GuiComponent(window)
	...
	, mGrid(window, Eigen::Vector2i(1, 2))
{
	...
	mMsg = std::make_shared<TextComponent>(mWindow, text, Font::get(FONT_SIZE_MEDIUM), COLOR_GRAY3, align);
